...
def load_icons():
    ...
    for name, (light_path, dark_path, size) in icon_configs.items():
        try:
            light_full_path = resource_path(light_path)
            dark_full_path = resource_path(dark_path)

            light_img = Image.open(light_full_path) if os.path.isfile(light_full_path) else None
            dark_img = Image.open(dark_full_path) if os.path.isfile(dark_full_path) else None
